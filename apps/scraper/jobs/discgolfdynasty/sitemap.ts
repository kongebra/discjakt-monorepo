...
async function scrapeDiscgolfDynastySitemap() {
  const baseUrl = "https://discgolfdynasty.no";
  const name = "Disc Golf Dynasty";
  const slug = "discgolfdynasty";
  ...
  try {
    ...
    logger.error("discgolfdynasty scraping error:", error);
  } finally {
...
(async () => {
  try {
    await scrapeDiscgolfDynastySitemap();
  } catch (error) {
