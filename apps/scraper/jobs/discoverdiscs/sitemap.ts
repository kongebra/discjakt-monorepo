...
async function scrapeDiscoverDiscsSitemap() {
  const baseUrl = "https://discoverdiscs.no";
  const name = "Discover Discs";
  const slug = "discoverdiscs";
  ...
  try {
    ...
    logger.error("discoverdiscs scraping error:", error);
  } finally {
...
(async () => {
  try {
    await scrapeDiscoverDiscsSitemap();
  } catch (error) {
